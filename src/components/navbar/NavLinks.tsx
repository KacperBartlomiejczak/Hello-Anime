...
export default function NavLinks({ isActive, onClick }: NavLinksProps) {
  return (
    <ul
      className={cn(
        "fixed top-0 left-0 h-screen w-full flex flex-col justify-center items-center bg-secondary-background transition-transform gap-4 z-40",
    ...
    >
      {dataLinks.map((link) => (
        <li key={link.id}>
          <Link
            href={link.href}
            className={cn(classes.link, "text-base md:text-lg")}
            onClick={onClick}
          >
            {link.title}
          </Link>
